...
Validator.prototype.validateField = function() {
  if(this.validateFieldActive() && this.validateFieldHelper() == 1) {
    this.showErrorField();
    return 1;
  } else {
    this.hideErrorField();
...
Validator.prototype.validateFieldActive = function() {
  if (this.fieldName.indexOf("_OtherSelection", this.fieldName.length - "_OtherSelection".length) != -1) {
    // OtherSelection validators are only applicable if "OtherSelection" is actually selected.
    // Doesn't apply if nothing is selected (outer selector validation rules will catch that if it matters)
    // or if a non-Other option is selected (in which case the content of the "Other" field is ignored).
    var selector = document.getElementById(this.fieldName.substring(0, this.fieldName.length - "_OtherSelection".length));
    if (selector.selectedIndex != -1 && selector.options[selector.selectedIndex].value == "OtherSelection") {
      return 1;
    } else {
      return 0;
    }
...
Validator.prototype.validateFieldHelper = function() {
    ...
    return 1; // fail everytime -- i.e., this is "abstract".
}
