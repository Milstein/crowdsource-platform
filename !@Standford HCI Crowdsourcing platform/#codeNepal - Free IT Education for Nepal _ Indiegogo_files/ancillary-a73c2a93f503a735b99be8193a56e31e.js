...
(function () {
  function PerkFactory ($http, $rootScope, _, countries, i18n, $log, gon) {
    ...
    function Perk (attributes, options) {
      ...
      // without this the responsive campaign page has big problems
...
(function () {
  'use strict';
  angular.module('utils').factory('currencies', ['gon', function (gon) {
    function CurrenciesService(initialCurrencies) {
      var currencies = initialCurrencies;

      this.setCurrencies = function (newCurrencies) {
    ...
    return new CurrenciesService((gon && gon.services && gon.services.currencies) || []);
  }]);
})();
...
/* global FB:false */
(function () {
  angular.module('utils').factory('fb', ['$q', 'gon', function ($q, gon) {
    var fb = {};
    ...
    fb.og_share = function(uri, options){
      ...
      //Get the right environment prefix for the custom action ('indiegogo-dev', 'indiegogo-stg' and 'indiegogo' for prod)
      var action_type = 'indiegogo' + namespace[gon.env] + ':share';
      var params = {
...
(function () {
  'use strict';
  angular.module('utils').factory('regions', ['gon', function (gon) {
    function RegionsService(initialRegions) {
      var regions;

      this.setRegions = function(rgns) {
        regions = rgns;
      };
      ...
      this.getStates = function() {
        return _.select(regions, function(region) {
          return region.code && region.code.match(/^STTE_US/);
        });
      };

      this.getProvinces = function() {
        return _.select(regions, function(region) {
          return region.code && region.code.match(/^STTE_CA/);
        });
      };
      ...
      this.byTwoLetterCode = function(code) {
        ...
        var state    = _.findWhere(regions, { code: 'STTE_US' + code });
        var province = _.findWhere(regions, { code: 'STTE_CA' + code });
    ...
    return new RegionsService(gon && gon.services && gon.services.regions || []);
  }]);
})();
